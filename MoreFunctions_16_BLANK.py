...
def top_3_movies():
    movie_list = []
    while True:
        user_input = input("type in t3 movies: ")
        movie_list = user_input.split(",")
        if len(movie_list) != 3:
            continue
        break
    _mov1 = movie_list[0]
    _mov2 = movie_list[1]
    _mov3 = movie_list[2]
    return _mov1, _mov2, _mov3

mov1, mov2, mov3 = top_3_movies()
print("top 3 movies are: ", mov1, mov2, mov3)
...
def add_hotel_rating(ratings, new_rating):
    ...
    average = sum(ratings) / total_num
    status = "Not reconmmended"
    if average > 7:
        status = "recommeneded"

    return total_num, average, status

total_rating_number, average_rating, recommendation = add_hotel_rating([7, 8.1], new_rating = 6)
print(f"total numbre of ratings: {total_rating_number}, average rating: {average_rating:.2f}, status: {recommendation}")